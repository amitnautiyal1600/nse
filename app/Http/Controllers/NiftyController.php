...
class NiftyController extends Controller
{
    ...
    private $api_url = "https://www.nseindia.com/api/option-chain-indices?symbol=NIFTY";
    private $headers = [
            'Content-Type' => 'application/json', 
        ]; 

    public function niftyHome() { 
        return view('nifty');
    }

    public function saveNiftyNseData() {
        $ch = curl_init(); 
        ...
        //Save Nifty NSE Data
        if($nseApiData) { 
            $nse_data_save = NiftyNseData::create([
                'nifty_expiry' => $filtered_nsedata->data[0]->expiryDate,
                'filtered_nifty_nse_data' => json_encode($filtered_nsedata),
                'all_nifty_nse_data' => json_encode($all_nsedata),
            ]);
        }
    ...
    public function getNiftyNseData(Request $request) {
        $ch = curl_init(); 
        curl_setopt($ch, CURLOPT_URL, $this->api_url); 
        curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
